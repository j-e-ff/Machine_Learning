...
# SPECIFICATION:
#   The program will use 3,823 training samples (optdigits.tra file) and 1,797 test samples (optdigits.tes file).
#   Read the file optdigits.names to get detailed information about this dataset, as well as the files
#   optdigits-orig.tra and optdigits-orig.names to see the original format of the data, and how it was transformed to
#   speed up the learning process. You will compare the model performances while varying two hyperparameters,
#   learning rate and shuffle, checking which combination leads to the best predictions
    ...
    for j in range(len(r)): #iterates over r
        ...
        for algorithm in ['Perceptron', 'MLP']: #iterates over the algorithms
            ...
            if algorithm == 'Perceptron' and accuracy > highest_perceptron_accuracy:
                ...
                print(f"Highest Perceptron accuracy so far:{accuracy:0.2f}, Parameters: learning rate= {learning_rate}, shuffle = {shuffle}")
            elif algorithm == 'MLP' and accuracy > highest_mlp_accuracy:
                highest_mlp_accuracy = accuracy
                print(f"Highest MLP accuracy so far:{accuracy:0.2f}, Parameters: learning rate= {learning_rate}, shuffle = {shuffle}")